...
low_odo, high_odo = iqr_bounds(df["Odometer_km"])
...
print(df[["CarAge", "Km_per_year", "Accident_Rate", "Is_OldCar", "Is_Urban", "LogPrice"]].head())

# 9. Feature Scaling (X only; keep targets & dummies unscaled)
dont_scale = {"Price", "LogPrice"}
numeric_cols = df.select_dtypes(include=["int64", "float64"]).columns.to_list()
exclude = [c for c in df.columns if c.startswith("Location_")] + ["Is_Urban"] + ["Is_OldCar"]
